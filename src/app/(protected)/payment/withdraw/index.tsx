...
export default function WithdrawScreen() {
  ...
  const overBalance = numeric > WALLET_BALANCE;
  const belowMin = numeric > 0 && numeric < MIN_AMOUNT;
  const canContinue = numeric >= MIN_AMOUNT && !overLimit && !overBalance;
  ...
  const pickQuick = (amount: number) => {
    if (amount > WALLET_BALANCE) return;
  ...
  const getHintNode = () => {
    if (overBalance)
      return <Text style={styles.limitError}>Exceeds available balance</Text>;
    if (overLimit)
      return <Text style={styles.limitError}>Exceeds daily limit of ₦5,000,000</Text>;
    if (belowMin)
      return <Text style={styles.limitError}>Minimum withdrawal is ₦500</Text>;
    return (
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      <View style = {styles.tabRow}>
        <TouchableOpacity style={styles.backBtn} onPress={() => router.push('/')} activeOpacity={0.7}>
      ...
      {/* <View style={styles.tabRow}>
        <TouchableOpacity
          style={styles.tab}
          onPress={() => router.replace('/payment/deposit')}
          activeOpacity={0.75}
        >
          <Text style={styles.tabText}>Deposit</Text>
        </TouchableOpacity>
        <View style={[styles.tab, styles.tabActive]}>
          <Text style={styles.tabTextActive}>Withdraw</Text>
        </View>
      </View> */}
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.screenSub}>
          Available: ₦
          {WALLET_BALANCE.toLocaleString('en-NG', { minimumFractionDigits: 2 })}
        </Text>
      </View>

      {/* ── Step Bar ── */}
      <StepBar active={1} mode="wit" />

      {/* ── Amount Display ── */}
      <View style={styles.amtArea}>
        <Text style={styles.amtHint}>HOW MUCH TO WITHDRAW?</Text>
        ...
        <View style={styles.amtRow}>
          ...
          <Text
            style={[
              styles.amtVal,
              !raw && styles.amtDim,
              (overLimit || overBalance) && styles.amtError,
            ]}
          >
            {displayAmt}
          </Text>
      ...
      <View style={styles.chips}>
        {QUICK_AMOUNTS.map(a => {
          const disabled = a > WALLET_BALANCE;
      ...
      <View style={styles.ctaWrap}>
        <TouchableOpacity
          ...
          onPress={() =>
            router.push({
              pathname: '/payment/withdraw/method',
              params: { amount: String(numeric) },
            })
          }
        >
