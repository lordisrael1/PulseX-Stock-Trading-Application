...
const EXCHANGE_META = {
  US: {
    ...
    brokerColor: '#7C1F1F', brokerBg: '#FFF5F5',
...
export default function WithdrawBrokerageReviewScreen() {
  ...
  const amount = parseFloat(params.amount ?? '500');
  const exchange = (params.exchange as Exchange) ?? 'US';

  const [confirming, setConfirming] = useState(false);

  const m = EXCHANGE_META[exchange];
  const bpAfter = m.existingBP - amount;
  const newWalletBal = m.walletBal + amount;
  ...
  const handleConfirm = async () => {
    setConfirming(true);
    try {
      // TODO: call your withdraw-brokerage API
      // const res = await withdrawFromBrokerage({ amount, exchange });
      await new Promise(r => setTimeout(r, 1800));

      const ref = `PX${Math.floor(Math.random() * 90000000 + 10000000)}`;

      router.replace({
        pathname: '/(protected)/brokerage/withdrawbrokeragedone',
    ...
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.topBar}>
        ...
        <View style={styles.topCenter}>
          <Text style={styles.topTitle}>Review withdrawal</Text>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.amtHero}>
          <Text style={styles.amtDir}>WITHDRAWING FROM BROKERAGE</Text>
          <Text style={styles.amtVal}>{m.symbol}{fmt(amount)}</Text>
          <Text style={styles.amtNote}>{m.brokerLabel} → {m.walletLabel}</Text>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.routeRow}>
            {/* FROM = brokerage (reversed from fund flow) */}
            <View style={styles.routeNode}>
              <Text style={[styles.routeDir, { color: '#991B1B' }]}>FROM</Text>
              ...
              <Text style={[styles.routeName, { color: m.brokerColor }]}>{m.brokerLabel}</Text>
            </View>
            ...
            <View style={[styles.routeNode, { alignItems: 'flex-end' }]}>
              ...
              <Text style={[styles.routeName, { textAlign: 'right' }]}>{m.walletLabel}</Text>
            </View>
          </View>

          <View style={styles.dBody}>
            <DetailRow label="Withdrawal amount" value={`${m.symbol}${fmt(amount)}`} />
            <DetailRow label="Withdrawal fee" value="Free" valueColor="#166634" />
            <DetailRow label="Settlement" value="Instant" />
            <DetailRow label="Buying power after" value={`${m.symbol}${fmt(bpAfter)}`} valueColor="#DC2626" />
          </View>

          <View style={styles.balRowHighlight}>
            <Text style={styles.balRowLabel}>New wallet balance</Text>
            <Text style={styles.balRowValue}>{m.symbol}{fmt(newWalletBal)}</Text>
          </View>
        </View>
        ...
        <View style={styles.warnBox}>
          <Svg width={16} height={16} viewBox="0 0 24 24" style={{ flexShrink: 0, marginTop: 1 }}>
            <Path d="M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20z" stroke="#991B1B" strokeWidth={1.8} fill="none" />
            <Path d="M9 9l6 6M15 9l-6 6" stroke="#991B1B" strokeWidth={1.8} strokeLinecap="round" fill="none" />
          </Svg>
          <Text style={styles.warnTxt}>
            Withdrawing reduces your buying power immediately. You won't be able to open new positions with this amount until you fund your brokerage again.
          </Text>
        </View>
        ...
        <TouchableOpacity
          ...
          activeOpacity={0.85}
        >
          {confirming ? <ActivityIndicator color="#fff" /> : <Text style={styles.ctaTxt}>Confirm withdrawal</Text>}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  stepDotActive: {
    ...
    shadowColor: '#7C1F1F', shadowOffset: { width: 0, height: 0 },
  ...
  balRowLabel: { fontSize: 12, fontWeight: '700', color: '#0A0A0A' },
  balRowValue: { fontSize: 15, fontWeight: '800', color: '#166634' },
  warnBox: {
    flexDirection: 'row', alignItems: 'flex-start', gap: 9,
    backgroundColor: '#FFF5F5', borderRadius: 14, borderWidth: 0.5, borderColor: '#FECACA',
    padding: 13, marginBottom: 18,
  },
  warnTxt: { flex: 1, fontSize: 12, color: '#991B1B', lineHeight: 18 },
  cta: { backgroundColor: '#7C1F1F', borderRadius: 14, paddingVertical: 16, alignItems: 'center', marginBottom: 10 },
