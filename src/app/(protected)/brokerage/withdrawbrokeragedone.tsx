/**
 * WithdrawBrokerageDoneScreen.tsx
 * Step 3 of 3 — Success confirmation after withdrawing brokerage
 * buying power to wallet.
 */
...
const EXCHANGE_META = {
  US: { symbol: '$', walletLabel: 'USD Wallet', brokerLabel: 'US Brokerage', existingBP: 1240.00, walletBal: 29.14 },
  NGX: { symbol: '₦', walletLabel: 'NGN Wallet', brokerLabel: 'NGX Brokerage', existingBP: 182340.00, walletBal: 44678.32 },
};
...
export default function WithdrawBrokerageDoneScreen() {
    ...
    <SafeAreaView style={styles.root}>
      ...
      <View style={styles.topBar}>
        <View style={styles.topCenter}>
          <Text style={styles.topTitle}>Withdrawal complete</Text>
          <Text style={styles.topSub}>Funds in your wallet</Text>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <Animated.View style={[styles.successWrap, { opacity, transform: [{ scale }] }]}>
          <View style={styles.successIcon}>
            <Svg width={30} height={30} viewBox="0 0 24 24">
              ...
              <Path d="M8 12l3 3 5-5" stroke="#DC2626" strokeWidth={2.2} strokeLinecap="round" fill="none" />
            </Svg>
          </View>
          <Text style={styles.successTitle}>Withdrawal complete</Text>
          <Text style={styles.successSub}>
            {m.symbol}{fmt(amount)} moved to your {m.walletLabel.toLowerCase()}.
          </Text>
        </Animated.View>

        <View style={styles.balRow}>
          <View style={[styles.balCard, styles.balCardRed]}>
            <Text style={[styles.balLabel, { color: '#991B1B' }]}>Buying power</Text>
            <Text style={[styles.balValue, { color: '#7C1F1F' }]}>{m.symbol}{fmt(bpAfter)}</Text>
            <Text style={styles.balDiffMinus}>-{m.symbol}{fmt(amount)}</Text>
          </View>
          <View style={[styles.balCard, styles.balCardGreen]}>
            <Text style={[styles.balLabel, { color: '#4CAF50' }]}>Wallet balance</Text>
            <Text style={[styles.balValue, { color: '#0F2419' }]}>{m.symbol}{fmt(newWalletBal)}</Text>
        ...
        <TouchableOpacity style={styles.cta} onPress={handleBackToBrokerage} activeOpacity={0.85}>
          <Text style={styles.ctaTxt}>Back to brokerage</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.ctaGhost} onPress={handleViewWallet} activeOpacity={0.75}>
          <Text style={styles.ctaGhostTxt}>View wallet ↗</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  successIcon: {
    width: 64, height: 64, borderRadius: 20, backgroundColor: '#FFF5F5',
  ...
  balCardRed: { backgroundColor: '#FFF5F5', borderColor: '#FECACA' },
