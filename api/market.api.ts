/**
 * market.api.ts  (was massiveApi.ts — rename/move to api/market.api.ts)
...
const _ohlcCache: Record<string, OHLCBar[]> = {};
...
export const SEED_PRICES: Record<string, number> = {
  ...
  CVX: 161, PEP: 181, KO: 63, ADBE: 483, WMT: 67, BAC: 39, MCD: 304, CRM: 315,
  CSCO: 49, ACN: 380, SPY: 538,
...
// ─── NGX ticker set (Massive/Polygon has no NGX data — always seeds) ──────────

const NGX_TICKER_SET = new Set([
  'DANGCEM', 'GTCO', 'ZENITHBANK', 'MTNN', 'ACCESSCORP', 'UBA',
  'FBNH', 'SEPLAT', 'BUAFOODS', 'AIRTELAFRI', 'TRANSCORP', 'OKOMUOIL',
  'TOTAL', 'NESTLE', 'FLOURMILL', 'NB', 'CADBURY', 'UNILEVER', 'GUINNESS', 'WAPCO',
]);

function isNGX(ticker: string): boolean {
  return NGX_TICKER_SET.has(ticker);
}

// ─── API: Daily Market Summary ────────────────────────────────────────────────
// GET /v2/aggs/grouped/locale/us/market/stocks/{date}
// NGX tickers are not on Polygon/Massive US market — they fall through to seedQuote.
// Replace the NGX branch with your own exchange source when available.

export async function fetchGroupedDay(tickers: string[]): Promise<QuoteItem[]> {
  // Separate US vs NGX tickers up front — no point hitting the API for NGX tickers
  const usTickers  = tickers.filter(t => !isNGX(t));
  const ngxTickers = tickers.filter(t =>  isNGX(t));

  // NGX: always seed (no live source yet)
  const ngxQuotes = ngxTickers.map(seedQuote);

  if (!usTickers.length) return ngxQuotes;

  const tickerSet = new Set(usTickers);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return [...usTickers.map(seedQuote), ...ngxQuotes];
    }
    ...
    const found   = usTickers.filter(t =>  map[t]);
    const missing = usTickers.filter(t => !map[t]);
...
/** Fetch prev-day bar for each ticker to compute prevClose and change% */
async function enrichWithPrevClose(
  ...
  for (let i = 0; i < tickers.length; i += BATCH) {
    ...
    const prevBars = await Promise.allSettled(
      slice.map(t =>
        fetchWithTimeout(
          `${BASE}/v2/aggs/ticker/${t}/prev?adjusted=true&apiKey=${MASSIVE_KEY}`,
          {},
          5000
        )
          .then(r => r.json())
...
export async function fetchIconsBatched(
  ...
  const needed = tickers.filter(t => !_iconCache[t] && !isNGX(t));
...
// NOTE: '1D' uses hourly bars; all other ranges use daily bars.

export async function fetchOHLC(
  ticker: string,
  period: '1D' | '1W' | '1M' | '3M' | '1Y'
): Promise<OHLCBar[]> {
  // NGX tickers have no live data — return seed immediately, no API hit
  if (isNGX(ticker)) {
    const daysMap = { '1D': 1, '1W': 7, '1M': 30, '3M': 90, '1Y': 365 };
    return seedCandles(ticker, daysMap[period]);
  }
...
// ─── Derived helpers (used by AnalyticsScreen and anywhere else) ──────────────

/**
 * Split quotes into top-n gainers and top-n losers by changePct.
 * Default n=2 → 4 cards total in the analytics screen movers grid.
 */
export function deriveMovers(
  quotes: QuoteItem[],
  n = 2
): { gainers: QuoteItem[]; losers: QuoteItem[] } {
  const sorted = [...quotes].sort((a, b) => b.changePct - a.changePct);
  return { gainers: sorted.slice(0, n), losers: sorted.slice(-n).reverse() };
}

/** Count advancing / unchanged / declining tickers. */
export function deriveBreadth(
  quotes: QuoteItem[]
): { adv: number; unc: number; dec: number } {
  return quotes.reduce(
    (acc, q) => {
      if (q.changePct > 0) acc.adv++;
      else if (q.changePct < 0) acc.dec++;
      else acc.unc++;
      return acc;
    },
    { adv: 0, unc: 0, dec: 0 }
